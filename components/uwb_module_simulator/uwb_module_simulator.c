#include "include/uwb_module_simulator.h"
#include "esp_log.h"
#include <string.h>
#include <ctype.h>
#include <stdio.h>
...
esp_err_t uwb_simulator_init(const uwb_simulator_uart_config_t* sim_uart_config) {
    ...
    g_sim_baud_rate = sim_uart_config->baud_rate; // 保存配置的波特率
    g_sim_actual_rx_buffer_size = sim_uart_config->rx_buffer_size; // 保存配置的接收缓冲区大小
    ...
    esp_err_t ret = uart_driver_install(g_sim_uart_port,
                                      ...
                                      20, &g_sim_uart_queue, 0); // 事件队列大小为20，不使用中断分配标志
    if (ret != ESP_OK) {
    ...
    task_created = xTaskCreate(uwb_sim_ranging_data_task, "sim_ranging_data_task", 2048, NULL, 5, &g_sim_ranging_data_task_handle); // 模拟测距数据生成任务
    if (task_created != pdPASS) {
    ...
    // 如果默认模式是AT指令模式，则初始时挂起测距任务；如果是测距模式，则让其运行
    if (g_sim_current_mode == UWB_MODE_AT_COMMAND) {
         vTaskSuspend(g_sim_ranging_data_task_handle);
    } 

    ESP_LOGI(TAG_SIM, "UWB模拟器初始化完成。默认模式: %s", g_sim_current_mode == UWB_MODE_RANGING ? "测距模式" : "AT指令模式");
    return ESP_OK;
}
...
{
    uart_event_t event;
    uint8_t* data = (uint8_t*) malloc(g_sim_actual_rx_buffer_size); // 使用配置中实际的接收缓冲区大小
    if (!data) {
    ...
    while(1){ 
        ...
        { // 从队列等待UART事件
            switch (event.type) 
            {
                case UART_DATA: // 接收到数据事件
                    {
                        ESP_LOGD(TAG_SIM, "UART_DATA event, size: %d", event.size); // 记录event.size
